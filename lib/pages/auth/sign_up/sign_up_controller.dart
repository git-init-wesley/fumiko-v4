...
class _AuthSignUpController extends _AuthSignUpModel with PopupController, AuthNavigationController {
  ...
  FormFieldValidator<String> get usernameValidator => (String? value) => value != null && value.trim().isNotEmpty && RegExps.username.hasMatch(value)
      ? null
      : (errorUsername = AppLocalizations.current.usernameInvalid + (kIsWeb ? '' : '\n\n${AppLocalizations.current.usernameTooltip}'));
  ...
  FormFieldValidator<String> get confirmPasswordValidator =>
      (String? value) => value != null && value.trim().isNotEmpty && value == passwordController.text ? null : (errorConfirmPassword = AppLocalizations.current.passwordNotEqual);
  ...
  Future<void> signUp() async {
    ...
    CoreUserAuth.signUp(
        ...
        whenComplete: (List<AppException> appExceptions) async {
          ...
          if (appExceptions.where((e) => e.code == AuthExceptions.usernameAlreadyTaken().code || e.code == AuthExceptions.emailAlreadyInUse().code).isNotEmpty) {
            await openPopup(
                title: AppLocalizations.current.warning,
                titleColor: Colors.orangeAccent,
                description: appExceptions.contains(AuthExceptions.usernameAlreadyTaken()) ? AppLocalizations.current.usernameAlreadyTaken : AppLocalizations.current.emailAlreadyInUse,
