...
class MenusButton extends StatelessWidget {
  const MenusButton({Key? key, this.height = 42, this.maxWidth = 320, required this.color, required this.iconData, required this.text, this.textSize, this.onPressed}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.center, children: [
          Icon(iconData, size: textSize ?? Theme.of(context).textTheme.titleLarge?.fontSize),
          Container(
              margin: const EdgeInsets.only(left: 4),
              child: Text(text.toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: textSize ?? Theme.of(context).textTheme.titleLarge?.fontSize)))
        ]));
  }
}
